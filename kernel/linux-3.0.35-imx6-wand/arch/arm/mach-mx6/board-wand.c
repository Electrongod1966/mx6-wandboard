...
#define WAND_FIRST_TEST         IMX_GPIO_NR(4, 5)
...
static const iomux_v3_cfg_t wand_i2c_pads[3][2] __initdata = {
	...
	MX6DL_PAD_KEY_COL3__I2C2_SCL ^ PAD_CTL_PUS_22K_UP ^ PAD_CTL_PUS_100K_UP,
	MX6DL_PAD_KEY_ROW3__I2C2_SDA ^ PAD_CTL_PUS_22K_UP ^ PAD_CTL_PUS_100K_UP,
	}, {
...
static void __init wand_board_init(void) {
	...
	wand_init_usb();
	wand_init_ipu();
	printk(KERN_INFO"booting with wandboard modified kernel from smartpathindia\n\r");
	wand_init_hdmi();
#ifdef GG
	gpio_request(WAND_FIRST_TEST, "smartpath_first");
	gpio_direction_output(WAND_FIRST_TEST, 1);
	gpio_set_value(WAND_FIRST_TEST, 1);
#endif
	wand_init_wifi();
	wand_init_bluetooth();
	wand_init_pm();
